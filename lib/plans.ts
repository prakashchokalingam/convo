...
export async function getUserPlanLimits(userId?: string): Promise<PlanLimits> {
  ...
  return {
    ...
    maxSeatsPerWorkspace:
      subscription.maxSeatsPerWorkspace ?? PLAN_CONFIGS[subscription.plan].maxSeatsPerWorkspace,
...
export async function canInviteToWorkspace(
  ...
  if (!planLimits.canInviteUsers) {
    return {
      allowed: false,
      reason:
        'Your plan does not support inviting team members. Upgrade to Pro or Enterprise to add team members.',
    };
  }
...
export async function getWorkspaceUsage(userId?: string) {
  ...
  const [planLimits, userWorkspaces] = await Promise.all([
    ...
    db.select({ count: count() }).from(workspaces).where(eq(workspaces.ownerId, targetUserId)),
  ]);
...
export async function getWorkspaceMemberUsage(workspaceId: string, userId?: string) {
  ...
  const addonSeats = subscription?.addonSeats || 0;
  const totalSeats =
    planLimits.maxSeatsPerWorkspace === -1 ? -1 : planLimits.maxSeatsPerWorkspace + addonSeats;
