  ...
  if (error && typeof error === 'object' && 'code' in error) {
    ...
    switch (dbError.code) {
      case '23505': // Unique violation
        return createErrorResponse('Resource already exists', 409, ErrorCodes.ALREADY_EXISTS);
      ...
      case '23502': // Not null violation
        return createErrorResponse('Required field is missing', 400, ErrorCodes.MISSING_FIELDS);
      default:
        return createErrorResponse(
...
export const CommonErrors = {
  unauthorized: () => createErrorResponse('Authentication required', 401, ErrorCodes.UNAUTHORIZED),

  forbidden: (message = 'Access denied') => createErrorResponse(message, 403, ErrorCodes.FORBIDDEN),

  notFound: (resource = 'Resource') =>
    createErrorResponse(`${resource} not found`, 404, ErrorCodes.NOT_FOUND),

  badRequest: (message = 'Invalid request') =>
    createErrorResponse(message, 400, ErrorCodes.VALIDATION_ERROR),

  conflict: (message = 'Resource already exists') =>
    createErrorResponse(message, 409, ErrorCodes.ALREADY_EXISTS),
  ...
  internal: (message = 'Internal server error') =>
    createErrorResponse(message, 500, ErrorCodes.INTERNAL_ERROR),
};
...
export function requirePermission(
  ...
  if (!hasPermission) {
    throw new ApiError(message, 403, ErrorCodes.INSUFFICIENT_PERMISSIONS);
  }
}
