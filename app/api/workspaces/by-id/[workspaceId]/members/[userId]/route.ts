...
export async function PUT(req: NextRequest, { params }: MemberParams) {
  try {
    ...
    const canManage = canManageRole(
      currentUserMember.role as 'owner' | 'admin' | 'member' | 'viewer',
      targetMember.role as 'owner' | 'admin' | 'member' | 'viewer'
    );

    if (!canManage) {
      return NextResponse.json({ error: "You cannot manage this member's role" }, { status: 403 });
    }
    ...
    // Log activity
    await ActivityLogger.memberJoined(workspaceId, currentUserId, targetUserId, role, req);
...
export async function DELETE(req: NextRequest, { params }: MemberParams) {
  try {
    ...
    // Log activity
    await ActivityLogger.memberRemoved(workspaceId, currentUserId, targetUserId, req);
