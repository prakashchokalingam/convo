...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!templateData.workspaceId) {
      ...
      return NextResponse.json(
        { error: 'Template workspace ID is missing for a non-global template' },
        { status: 500 }
      );
    }
    ...
    const canEditTemplate = await checkWorkspacePermission(
      userId,
      templateData.workspaceId,
      'templates',
      'edit'
    );
    if (!canEditTemplate) {
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const canDeleteTemplate = await checkWorkspacePermission(
      userId,
      templateData.workspaceId,
      'templates',
      'delete'
