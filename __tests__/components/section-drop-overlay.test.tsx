...
vi.mock('framer-motion', () => ({
  motion: {
    div: ({ children, className, initial, animate, exit, transition, layout, layoutId, ...props }: any) => (
...
describe('Section Drop Zone Visual Feedback', () => {
  ...
  describe('Section Drop Target Styling', () => {
    it('should show blue overlay when dragging field over section', async () => {
      ...
      const sectionElement = container.querySelector('[data-rbd-droppable-id*="section-drop-section-1"]')
      
      // If the specific droppable isn't found, look for the section container
      const sectionContainer = container.querySelector('div:has(input[value="Contact Information"])')
    ...
    it('should support section collapse/expand functionality', async () => {
      ...
      const collapseButton = collapseButtons.find(btn => 
        btn.querySelector('svg') // Looking for chevron icon
      )
