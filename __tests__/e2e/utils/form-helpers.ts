...
export class FormBuilder {
  ...
  async createFormWithAI(prompt: string): Promise<string> {
    ...
    await this.page.waitForSelector('[data-testid="form-preview"], [data-testid="form-fields"]', {
      timeout: 30000,
    });
  ...
  async createFormManually(formData: TestForm): Promise<string> {
    ...
    if (formData.description) {
      await this.page.fill(
        '[data-testid="form-description-input"], [name="description"]',
  ...
  async addField(fieldType: string, label: string, required: boolean = false) {
    ...
    await this.page.waitForSelector(
      `[data-testid="field-${label}"], [data-field-label="${label}"]`
    );
  }
  ...
  async toggleConversationalMode() {
    await this.page.click(
      '[data-testid="conversational-toggle"], [data-testid="form-mode-toggle"]'
    );
  ...
  async previewForm() {
    ...
    await this.page.waitForSelector(
      '[data-testid="form-preview-modal"], [data-testid="form-preview"]'
    );
  }
...
export class FormSubmitter {
  ...
  async submitStaticForm(submissionData: SubmissionData) {
    ...
    // Submit the form
    await this.page.click(
      '[type="submit"], [data-testid="submit-button"], button >> text="Submit"'
    );
  ...
  async submitConversationalForm(submissionData: SubmissionData) {
    ...
    await this.page.waitForSelector(
      '[data-testid="chat-interface"], [data-testid="conversation-form"]'
    );
  ...
  private async fillCurrentConversationalField(value: string | boolean | string[]) {
    ...
    const activeInput = this.page
      .locator('[data-testid="current-input"], .active input, .current input')
      .first();
  ...
  async verifySubmissionSuccess() {
    await expect(
      this.page.locator(
        '[data-testid="success-message"], text="Thank you", text="submitted successfully"'
      )
    ).toBeVisible();
  }
  ...
  async verifyValidationErrors(expectedErrors: string[]) {
    for (const error of expectedErrors) {
      await expect(
        this.page.locator(`text="${error}", [data-testid="error-message"]`)
      ).toBeVisible();
    }
  }
}
...
export class FormAnalytics {
  ...
  async exportResponses(format: 'csv' | 'json' = 'csv') {
    ...
    await this.page.click(
      `[data-testid="export-${format}"], button >> text="${format.toUpperCase()}"`
    );
...
export async function assertFormFieldExists(page: Page, fieldLabel: string) {
  await expect(
    page.locator(`label:has-text("${fieldLabel}"), [data-field-label="${fieldLabel}"]`)
  ).toBeVisible();
}

export async function assertFormModeToggle(page: Page, mode: 'static' | 'conversational') {
  const toggle = page.locator(
    '[data-testid="form-mode-toggle"], [data-testid="conversational-toggle"]'
  );
