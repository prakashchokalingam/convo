...
test.describe('Authentication Flow', () => {
  test('should complete signup and onboarding flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="workspace-name"], h1')).toContainText(testWorkspace.name);
    await expect(page.locator('[data-testid="user-menu"], [data-testid="user-avatar"]')).toBeVisible();
    await expect(page.locator('[data-testid="create-form-button"], button >> text="Create Form"')).toBeVisible();
  });
  ...
  test('should handle login flow for existing user', async ({ page }) => {
    ...
    await navigator.waitForAuthRedirect();
    await expect(page.locator('[data-testid="workspace-dashboard"], [data-testid="user-menu"]')).toBeVisible();
  });
  ...
  test('should validate signup form properly', async ({ page }) => {
    ...
    await expect(page.locator('text="Password is required", [data-error*="password"]')).toBeVisible();
