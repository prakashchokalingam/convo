...
export function TemplateCard({
      ...
      <Card className={`${variant === 'compact' ? 'h-auto' : 'h-full'}`}>
        ...
        <CardContent className={`${variant === 'compact' ? 'py-3' : 'py-4'} space-y-3`}>
          ...
          <div className={`pt-2 ${variant === 'compact' ? 'space-y-1' : 'space-y-1.5'}`}>
            <Skeleton className={`h-3 ${variant === 'compact' ? 'w-3/5' : 'w-4/6'}`} />{' '}
            {/* Created At / By */}
  ...
  return (
    ...
    >
      ...
      <CardContent className={`${variant === 'compact' ? 'py-3' : 'py-4'} space-y-3`}>
        {' '}
        {/* Adjusted space-y for new metadata section */}
        ...
        {/* Template Metadata */}
        <div
          className={`pt-2 text-xs text-muted-foreground space-y-1 ${variant === 'compact' ? 'mt-2' : 'mt-3'}`}
        >
          {variant === 'compact' ? (
            <>
              <div>
                ...
                {template.isGlobal ? (
                  ...
                  <span className='font-medium'>
                    By: User {template.createdBy?.substring(0, 8) || 'N/A'}
                  </span>
                )}
              </div>
            </>
              ...
              <div>
                ...
                {template.isGlobal ? (
                  ...
                  <span className='font-medium text-foreground'>
                    User {template.createdBy?.substring(0, 8) || 'N/A'}
                  </span>
                )}
              </div>
              <div>
