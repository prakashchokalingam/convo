...
function FieldPreview({ field, value, onChange, isVisible }: FieldPreviewProps) {
  const renderField = () => {
    switch (field.type) {
      ...
      case 'select':
        return (
          <Select value={value || ''} onValueChange={onChange} disabled={!isVisible}>
            <SelectTrigger>
      ...
      case 'switch':
        return (
          <div className='flex items-center space-x-2'>
            <Switch checked={!!value} onCheckedChange={onChange} disabled={!isVisible} />
  ...
  return (
    <div className={cn('space-y-2 transition-opacity duration-200', !isVisible && 'opacity-40')}>
      ...
      {field.hint && <p className='text-xs text-gray-600'>{field.hint}</p>}

      <div className={cn(!isVisible && 'pointer-events-none')}>{renderField()}</div>
    </div>
  );
}
...
export function ConditionalPreview({ formConfig, className, onClose }: ConditionalPreviewProps) {
  ...
  const { values, visibilityActions, updateValue, getVisibleValues } =
      ...
      <CardHeader>
        ...
        {conditionalFields.length > 0 && (
          <div className='text-sm text-gray-600'>
            Found {conditionalFields.length} field{conditionalFields.length > 1 ? 's' : ''} with
            conditional logic
          </div>
        )}
      </CardHeader>
