...
export function DependencyIndicator({ field, allFields, className }: DependencyIndicatorProps) {
  ...
  const dependents: FieldDependency[] = useMemo(() => {
    return allFields
      .filter(f => f.conditional?.conditions?.some(condition => condition.fieldId === field.id))
      .map(f => ({
        id: f.id,
        label: f.label,
      ...
      <div className={cn('flex items-center gap-1', className)}>
        ...
        {hasDependencies && (
          <Tooltip>
            ...
            <TooltipContent side='top'>
              <p>
                Depends on {dependencies.length} field{dependencies.length > 1 ? 's' : ''}:{' '}
        ...
        {hasDependents && (
          <Tooltip>
            ...
            <TooltipContent side='top'>
              <p>
                {dependents.length} field{dependents.length > 1 ? 's' : ''} depend
                {dependents.length === 1 ? 's' : ''} on this:{' '}
