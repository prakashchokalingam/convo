...
interface FieldWrapperProps {
  ...
  index: number;
  sectionId?: string;
  onDrop?: (droppedItem: DragItem, position: 'before' | 'after') => void;
}

function FieldWrapper({ children, fieldId, index, sectionId, onDrop: _onDrop }: FieldWrapperProps) {
  // Before drop zone
  ...
  return (
    <div className='relative'>
      {/* Before drop zone */}
      <div ref={setBeforeRef} className='absolute -top-3 left-0 right-0 h-6 z-10'>
      ...
      {/* After drop zone */}
      <div ref={setAfterRef} className='absolute -bottom-3 left-0 right-0 h-6 z-10'>
...
function SectionContainer({
  ...
  const sectionField = section as FieldConfig & { isCollapsed?: boolean; description?: string };
  ...
  const handleToggleCollapse = () => {
    onUpdateField(section.id, { isCollapsed: !isCollapsed } as Partial<FieldConfig>);
  };
    ...
    >
          ...
          <div className='text-center bg-white px-6 py-3 rounded-full border-2 border-blue-500 shadow-lg'>
            <div className='flex items-center space-x-3'>
              ...
              <span className='text-blue-700 font-semibold'>
                Add field to &quot;{section.label}&quot; section
              </span>
            </div>
          </div>
        </motion.div>
      ...
      >
        ...
        <div className='flex items-center space-x-1 ml-4'>
          ...
          <Button variant='ghost' size='sm' onClick={handleSectionClick} className='h-8 w-8 p-0'>
              ...
              >
                <AnimatePresence mode='popLayout'>
                  {children.map((field, index) => (
                    ...
                    >
                      <FieldWrapper fieldId={field.id} index={index} sectionId={section.id}>
...
export function FormCanvas({
  ...
  const getFieldsBySection = useMemo(() => {
    ...
    config.fields
      ...
      .forEach(field => {
        if (field.sectionId && !sectionFields[field.sectionId]) {
  ...
  const renderField = useCallback(
    (field: FieldConfig, index: number) => {
      if (field.type === 'section') {
        ...
        return (
          <FieldWrapper key={field.id} fieldId={field.id} index={index}>
            <SectionContainer
              section={field}
              ...
              <CardContent className='p-6'>
                <div className='space-y-6'>
                  ...
                  <div className='pb-6 border-b border-border'>
                    <h1 className='text-2xl font-bold mb-2'>{config.settings.title}</h1>
                    {config.description && (
                  ...
                  <div className='pt-6 border-t border-border'>
                    <Button className='w-full md:w-auto'>{config.settings.submitButtonText}</Button>
                  </div>
                </div>
              </CardContent>
            )}
