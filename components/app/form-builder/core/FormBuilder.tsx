...
import { LayoutGrid , Save, Eye, Settings, Undo, Redo, Zap } from 'lucide-react';
...
export function FormBuilder({
  ...
  const [activeTab, setActiveTab] = useState('canvas'); // 'fields', 'canvas', 'properties'

  // Effect to reset activeTab when screen size changes (e.g., from mobile to desktop)
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 768) {
        // md breakpoint
        setActiveTab('canvas'); // Reset to canvas on desktop
      }
    };
  ...
  const addField = useCallback(
    (fieldType: FieldType, sectionId?: string, position?: number) => {
  ...
  const duplicateField = useCallback(
    (field: FieldConfig) => {
      ...
      if (field.sectionId) {
        ...
        if (sectionIndex !== -1) {
          const section = newFields[sectionIndex] as FieldConfig & { children?: string[] };
  ...
  const addFieldToSectionAtPosition = useCallback(
    (fieldType: FieldType, sectionId: string, position: number) => {
      const newField = createFieldConfig(fieldType, {
        sectionId,
        nestingLevel: 1,
  ...
  const moveFieldToSectionAtPosition = useCallback(
    (fieldId: string, sectionId: string, position: number) => {
      ...
      const otherFields = state.config.fields.filter(
        f => f.id !== fieldId && f.sectionId !== sectionId
      );
      const sectionFields = state.config.fields.filter(
        f => f.id !== fieldId && f.sectionId === sectionId
      );
  ...
  const moveFieldToMainFormAtPosition = useCallback(
    (fieldId: string, position: number) => {
      ...
      const newFields = [...mainFields, ...sectionFields].map((field, index) => ({
        ...field,
        order: index,
      }));
  ...
  const reorderFieldAtPosition = useCallback(
    (fieldId: string, newPosition: number, sectionId?: string) => {
      if (sectionId) {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      switch (overItem?.type) {
        ...
        case 'field-insertion': {
          ...
          const { position, index, sectionId } = overItem;
    ...
    [
      ...
      updateField,
      moveField,
      addFieldToSectionAtPosition,
      moveFieldToSectionAtPosition,
      moveFieldToMainFormAtPosition,
      reorderFieldAtPosition,
    ]
  );
  ...
  const hasConditionalFields = useMemo(() => {
    return state.config.fields.some(
      field => field.conditional && field.conditional.conditions.length > 0
    );
  }, [state.config.fields]);
      ...
      <div className='flex flex-col h-screen bg-gray-50'>
        ...
        <div className='flex items-center justify-between p-4 bg-white border-b border-gray-200 shadow-sm'>
          <div className='flex items-center space-x-4'>
            <h1 className='text-xl font-semibold text-gray-900'>Preview: {state.config.name}</h1>
          </div>
              ...
              <div className='space-y-6'>
                ...
                <div className='pb-6 border-b border-gray-100'>
                  ...
                  {state.config.description && (
                    <p className='text-gray-600 text-lg'>{state.config.description}</p>
                  )}
                </div>
  ...
  return (
    <div className={`flex flex-col h-screen bg-gray-50 ${className}`}>
      ...
      <div className='flex items-center justify-between p-4 bg-white border-b border-gray-200 shadow-sm'>
        <div className='flex items-center space-x-4'>
          <h1 className='text-xl font-semibold text-gray-900'>{state.config.name}</h1>
        ...
        <div className='flex items-center space-x-2'>
          {hasConditionalFields && (
            ...
            >
              ...
              <span className='hidden md:inline'>Test Logic</span>
            </Button>
          )}
          <Button
            variant='outline'
      ...
      {showPreview ? (
        <div className='flex-1 p-4 overflow-auto'>
          <ConditionalPreview formConfig={state.config} onClose={() => setShowPreview(false)} />
        </div>
      ) : (
        <div className='flex flex-1 overflow-hidden'>
          <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            {/* Left Panel - Field Library (Desktop) / Tabbed (Mobile) */}
            ...
            {/* Right Panel - Properties (Desktop) / Tabbed (Mobile) */}
      ...
      <div className='md:hidden flex-shrink-0 bg-white border-t border-gray-200 p-2 shadow-lg'>
        <SegmentedControl value={activeTab} onValueChange={setActiveTab} className='w-full'>
          <SegmentedControl.Item value='fields' className='flex-1'>
            <LayoutGrid className='h-4 w-4 mr-2' /> Fields
          </SegmentedControl.Item>
          <SegmentedControl.Item value='canvas' className='flex-1'>
            <Eye className='h-4 w-4 mr-2' /> Canvas
          </SegmentedControl.Item>
          <SegmentedControl.Item value='properties' className='flex-1'>
